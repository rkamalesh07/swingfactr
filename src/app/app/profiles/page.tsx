...
function PlayerModal({ player, onClose }: { player: Player; onClose: () => void }) {
  ...
  return (
    <div onClick={(e) => { if (e.target === e.currentTarget) onClose() }}
      ...
      <div style={{ background: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: '6px',
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #111',
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none',
            cursor: 'pointer', color: '#444', fontSize: '20px', padding: '4px', lineHeight: 1 }}>
            ×
          </button>
        </div>
              ...
              <div>
                ...
                {[
                  ...
                  <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '16px',
                    ...
                    <span style={{ fontSize: '9px', color: '#333', letterSpacing: '0.1em', width: '28px' }}>
                      {label}
                    </span>
                    ...
                    {vals.length > 0 && (
                      <>
                        ...
                        <span style={{ fontSize: '9px', color: '#333' }}>last</span>
                        {vals.length > 1 && (
                ...
                <div style={{ marginTop: '12px', fontSize: '8px', color: '#1a1a1a', lineHeight: 1.8 }}>
            ...
            logLoading ? (
              ...
              <div style={{ overflowX: 'auto' }}>
                <div style={{ display: 'grid',
                  ...
                  padding: '6px 0', fontSize: '8px', color: '#2a2a2a',
                  letterSpacing: '0.1em', borderBottom: '1px solid #111', minWidth: '520px' }}>
                ...
                {gamelog.map((g, i) => (
                  <div key={i} style={{ display: 'grid',
                    ...
                    <span style={{ fontSize: '9px', color: '#333' }}>{String(g.game_date).slice(5)}</span>
                    <span style={{ fontSize: '10px', color: '#444' }}>{g.is_home ? '' : '@'}{g.opponent_abbr}</span>
                    <span style={{ fontSize: '10px', color: '#555' }}>{Math.round(g.minutes)}</span>
                    ...
                    <span style={{ fontSize: '11px', color: '#555' }}>{g.stl}</span>
                    <span style={{ fontSize: '11px', color: '#555' }}>{g.blk}</span>
                    <span style={{ fontSize: '11px', color: '#555' }}>{g.fg3m}</span>
                  </div>
                ))}
              </div>
            )
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080808', color: '#888', fontFamily: MONO }}>
      ...
      <div style={{ borderBottom: '1px solid #0f0f0f', padding: '12px 24px',
        ...
        <Link href="/" style={{ color: '#333', textDecoration: 'none', fontSize: '11px' }}>← HOME</Link>
        <span style={{ color: '#1a1a1a' }}>·</span>
        <span style={{ color: '#333', fontSize: '11px', letterSpacing: '0.1em' }}>PLAYER PROFILES · 2025–26</span>
        {!loading && <span style={{ color: '#222', fontSize: '11px' }}>{filtered.length} players</span>}
      </div>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '28px 24px' }}>
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search player..."
              style={{ padding: '8px 12px', background: '#0a0a0a', border: '1px solid #1a1a1a',
                borderRadius: '4px', outline: 'none', fontFamily: MONO, fontSize: '12px',
                color: '#e0e0e0', width: '200px' }} />
            <select value={teamFilter} onChange={e => setTeamFilter(e.target.value)}
              style={{ padding: '8px 10px', background: '#0a0a0a', border: '1px solid #1a1a1a',
        ...
        {loading ? (
          <div style={{ padding: '60px', textAlign: 'center', fontSize: '11px', color: '#222' }}>Loading players...</div>
        ) : (
          <div style={{ background: '#0a0a0a', border: '1px solid #111', borderRadius: '6px', overflow: 'auto' }}>
              ...
              <tbody>
                {filtered.map((p, i) => (
                  <tr key={`${p.player_name}-${i}`} onClick={() => setSelected(p)}
                    ...
                    <td style={{ padding: '10px 8px', fontSize: '10px', color: '#444', textAlign: 'center' }}>
                      {p.team_abbr}
                    </td>
                    <td style={{ padding: '10px 8px', fontSize: '10px', color: '#333', textAlign: 'center' }}>
                      {p.position || '—'}
                    </td>
                    {SORT_OPTIONS.map(s => {
