...
def detect_archetype(raw: dict) -> str:
    ...
    if is_big and ppg >= 18 and apg >= 5.0:    return "Point Forward"
    if is_big and ppg >= 15 and apg >= 4.5 and ast_rate > 0.16: return "Point Forward"
...
def build_league(players: list[dict], adv_lookup: dict = None, contracts: dict = None) -> dict:
    ...
    for p in fa_pool:
        k = _get_2k_ovr(p.get("name",""), _r2k)
        if k: p["overall"] = k
    return {"teams": teams, "fa_pool": fa_pool, "season": 1, "day": 362, "games_simmed": 0}
...
TEAM_STATUS_MAP = {
    ...
    "PHX":"Retooling","POR":"Retooling","UTAH":"Retooling","DAL":"Retooling",
    "LAC":"Retooling",
